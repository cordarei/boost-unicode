...
write_entry::write_entry()
{
    ...
    sort_data2 = 0;
	uppercase = (codepoint)0;
	lowercase = (codepoint)0;
	titlecase = (codepoint)0;
	has_complex_case = false;
	line_break = (break_class::type)-1;
	joining = (join_type::type)-1;
}

write_entry::write_entry(codepoint ch, const character_properties& data)
{
	chr = ch;
	name = data.name;
    ...
    word_boundary_kind = data.word_boundary_kind;
    sentence_boundary_kind = data.sentence_boundary_kind;
...
void write_entry::calc_properties_checksum(boost::crc_basic<16>& crc)
{
	...
	crc.process_bytes(&word_boundary_kind, sizeof(word_boundary_kind) );
	crc.process_bytes(&sentence_boundary_kind, sizeof(sentence_boundary_kind) );
...
bool write_entry::has_same_properties(const write_entry& other) const
{
	...
	return general_category == other.general_category && 
		...
		word_boundary_kind == other.word_boundary_kind &&
        sentence_boundary_kind == other.sentence_boundary_kind && 
...
std::fstream& operator << (std::fstream& file, const write_entry& data)
{
	...
	ss << "\t\tword_boundary::" << as_string(data.word_boundary_kind) << ",\n";
	...
	ss << "\t\tsentence_boundary::" << as_string(data.sentence_boundary_kind) << ",\n";
...
decomp_entry::decomp_entry(codepoint ch, const std::vector<codepoint>& decomp)
{
	chr = ch;
...
std::fstream& operator << (std::fstream& file, const decomp_entry& data)
{
	// create the data in a stringstream
	std::stringstream ss;
	ss << "static const codepoint __uni_decomp_data_0x" << std::hex << data.chr << "[] = {\t";
...
std::fstream& operator << (std::fstream& file,
	const tuple<codepoint, std::vector<complex_casing> >& data)
{
	// write the block
...
std::fstream& write_block::write_complex_case_block(std::fstream& file,
	const std::vector<tuple<codepoint, std::vector<complex_casing> > >&
...
void write_block_tables_and_blocks(const std::vector<read_block>& tbl_block,
	const std::vector<codepoint>& tbl_block_ident, 
	const char * dest_path, size_t nFiles)
{
	...
	file << "#include <boost/unicode/uni_character_properties.hpp>\n";
	file << "#include <boost/unicode/ucd/uni_ucd_character_properties.hpp>\n";
	file << "#include \"../data_parser/uni_impl_format_internal.hpp\"\n";
	size_t n;
	for (n = 0; n < nFiles; n++)
	{
	...
	const std::vector<read_block>& tbl_block, const char * dest_path_ucd_props)
{
	std::cout << " writing " << dest_path_ucd_props
		<< "uni_ucd_character_properties.hpp" << std::endl;

	// open file
	std::stringstream fss;
	fss << dest_path_ucd_props;
	fss << "uni_ucd_character_properties.hpp";
...
*******************************************************************/
void get_default_sort_characteristics(codepoint cp, collation_entry & sort_entry)
{
...
bool decompose_for_sort(const character_properties & props_char, 
                        const std::map <codepoint, character_properties> & props,
                                            std::vector<collation_data> & tbl_coll)
{
    std::vector <codepoint>::const_iterator iter = props_char.decomposition.begin();
    std::vector <codepoint>::const_iterator end = props_char.decomposition.end();
    for (; iter != end; iter++)
    {
        codepoint cp_decomp = *iter;
        const std::map <codepoint, character_properties>::const_iterator iter_char = 
...
*******************************************************************/
bool optimise_char_sort_data(const std::map <codepoint, character_properties> & props,
                                  codepoint cp, character_properties & props_char_var)
{
...
*******************************************************************/
void pack_char_sort_data(codepoint cp, character_properties & props_char_var)
{
    // sanity checks
    assert(props_char.sort_type == sort_type::is_index);
    assert(props_char.sort_set_index_or_data == (uint16_t)-1);
                            ...
                            props_char_var.sort_data[0].data.size() == 1)
    {
        ...
        assert(props_char.sort_data[0].following_chars.empty());
        ...
        else
        {
                ...
                coll.weight4 == sort_entry.data[0].weight4)
            {
                // default
                props_char_var.sort_variable = coll.variable;
                props_char_var.sort_type = sort_type::default;
                props_char_var.sort_index_or_data1 = 0;
                props_char_var.sort_data2 = 0;
                props_char_var.sort_data.clear();
            }
						...
						const std::vector <read_block>& blocks, write_data& data)
{
	...
	write_block block;
	for (codepoint cp = 0; cp <= 0x10FFFD; cp++)
	{	
		std::map <codepoint, character_properties>::iterator iter_char = 
                                                                    props.find(cp);
        ...
        if (props_char.sort_type == sort_type::is_index)
        {
            ...
            for (size_t n = 0; n < props_char.sort_data.size(); n++)
            {
                ...
                vector<uint16_t>::const_iterator iter_entry;
                vector<uint16_t>::const_iterator end_entry = tbl_data.end();
...
static codepoint cp_last_sort_data_entry = 0xFFFFF;

std::fstream& operator << (std::fstream& file, 
                           const tuple<codepoint, sort_data_entry>& data)
{
	// write the entry
...
void write_sort(const write_data& data, const char * dest_path)
{
    ...
    std::vector<tuple<codepoint, bool, codepoint> >::const_iterator iter_sf = 
                                            data.tbl_sort_following_chars.begin();
    std::vector<tuple<codepoint, bool, codepoint> >::const_iterator end_sf = 
                                            data.tbl_sort_following_chars.end();

    file << "static const codepoint __uni_sort_follow[] = {\n";
    ...
    {
        const tuple<codepoint, bool, codepoint> & entry = *iter_sf;
        if (get<1>(entry))
        {
    ...
    std::vector<tuple<codepoint, sort_data_entry> >::const_iterator iter_se = 
                                            data.tbl_sort_entries.begin();
    std::vector<tuple<codepoint, sort_data_entry> >::const_iterator end_se = 
