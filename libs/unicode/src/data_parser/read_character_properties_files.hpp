		...
		namespace ucd
		{
			...
			class syntax_error : std::runtime_error
			{
				static std::string make_message (
					const char* filename,
					const boost::spirit::file_iterator<> & first,
					const boost::spirit::file_iterator<> & last);
			public:
			...
			boost::spirit::file_iterator<> get_file_iterator
				(const char * name);

			void eat_comments (boost::spirit::file_iterator<> & current,
							   const boost::spirit::file_iterator<> & last);

			/// Read from UnicodeData.txt
			void read_character_properties_unicodedata (
				std::map <codepoint, character_properties> & prop, const char* filename, bool required);
