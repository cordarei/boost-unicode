...
tuple<codepoint, character_properties> read_one_character
...
void read_character_properties_unicodedata
	(std::map <codepoint, character_properties> & props, const char* filename, bool required)
{
	...
	do
	{
		...
		if (prop.name.find (", First") != std::string::npos)
		{
			...
			do
			{
				...
				{
						std::stringstream ss;
					ss << "Duplicate codepoint(s) in UnicodeData.txt. "
						"The curr duplicate codepoint is " << std::hex <<
		...
		{
			std::map <codepoint, character_properties>::const_iterator preExisting = props.find(character);
